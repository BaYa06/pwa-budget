...
async function getSettings(){
  return (await query(`select * from settings order by id asc limit 1;`)).rows[0];
}
...
module.exports = async (req, res) => {
  ...
  const startYear = startOfWeek(new Date(today.getFullYear(),0,1), 1);
  const weekNum = Math.ceil((today - startYear) / (7*24*3600*1000));
  const periodKey = isWeek ? `${today.getFullYear()}-W${weekNum}`
