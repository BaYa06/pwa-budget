...
module.exports = async (req, res) => {
  if (req.method === 'GET'){
    ...
    const sql = `select p.*, g.name as group_name, g.color as group_color from planned p left join groups g on g.id=p.group_id ${where} order by coalesce(next_due_date, now()) asc, id desc`;
  ...
  if (req.method === 'POST'){
    ...
    req.on('end', async ()=>{
      try{
        const { plan_type, title, amount, groupId, first_date=null, rule_freq=null, rule_n=null, next_due_date=null, auto_post=false, remind=false, note=null } = JSON.parse(body||'{}');
  ...
  if (req.method === 'PUT'){
    ...
    req.on('end', async ()=>{
      try{
        ...
        const { rows } = await query(`update planned set
