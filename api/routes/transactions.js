...
module.exports = async (req, res) => {
  if (req.method === 'GET'){
    ...
    const sql = `select t.*, g.name as group_name, g.color as group_color
                 from transactions t left join groups g on g.id=t.group_id
                 ${where} order by date desc, created_at desc limit 500`;
    ...
    req.on('end', async ()=>{
      try{
        ...
        const { rows } = await query(
          `insert into transactions(type,title,date,amount,group_id,comment)
