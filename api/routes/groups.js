...
module.exports = async (req, res) => {
  if (req.method === 'GET'){
    ...
    const { rows } = await query(
      `select * from groups where ($1 or archived=false) order by created_at desc`,
      [includeArchived]
    );
  ...
  if (req.method === 'POST'){
    ...
    req.on('end', async ()=>{
      try{
        ...
        const { rows } = await query(
          `insert into groups(name, comment, color) values ($1,$2,$3) returning *`,
          [name, comment, color]
        );
  ...
  if (req.method === 'PUT'){
    ...
    req.on('end', async ()=>{
      try{
        ...
        const { rows } = await query(
          `update groups set name=coalesce($2,name), comment=$3, color=$4, archived=coalesce($5, archived)
