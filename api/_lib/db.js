// Simple Postgres pool and schema bootstrap for Vercel serverless
...
function getPool() {
  if (!pool) {
    ...
    // For Neon/Vercel serverless, sslmode=require in URL is recommended
    pool = new Pool({
      connectionString: conn,
      // Rely on sslmode in URL; otherwise uncomment below
...
async function ensureSchema() {
  ...
  try {
    ...
    create table if not exists planned (
      ...
      group_id int references groups(id),
      first_date date,
      rule_freq text, -- daily|weekly|monthly|yearly|everyN
      rule_n int,
      next_due_date date,
